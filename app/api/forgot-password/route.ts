...
const transporter = nodemailer.createTransport({
  host: "smtp.zoho.com",   // Hardcoded Zoho SMTP host
  ...
  auth: {
    ...
    const resetUrl = `${process.env.NEXT_PUBLIC_API_URL}/reset-passwords?token=${token}&email=${email}`;

    await transporter.sendMail({
      from: `"16Zips" <${process.env.EMAIL_USER}>`,
      to: email,
    ...
    return NextResponse.json({
      message: "If this email exists in our system, you’ll receive a password reset link from 16Zips shortly.",
    });
  } catch (err) {
    console.error(err);
    return NextResponse.json(
