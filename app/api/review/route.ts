...
export async function GET(req: Request): Promise<Response> {
  try {
    ...
    const hasLimit = url.searchParams.has("_limit"); // 👈 Detect if _limit is passed
    const _page = parseInt(url.searchParams.get("_page") || "1");
    const _limit = hasLimit ? parseInt(url.searchParams.get("_limit") || "10") : 0; // If not provided, 0 = fetch all
    ...
    // ✅ CASE: Get reviews by slug (with or without pagination)
    if (slug) {
      ...
      if (hasLimit) {
        cursor.skip((_page - 1) * _limit).limit(_limit);
      }
      ...
      // 📊 Calculate full rating stats (always get all for rating summary)
    ...
    // ✅ CASE: Get all reviews (paginated or not)
    const query = {};
    const cursor = reviewsCollection.find(query).sort({ [_sort]: _order });
...
export async function PUT(req: Request): Promise<Response> {
  try {
    ...
    if (!id || !customerName || !reviewContent || isNaN(rating)) {
      return NextResponse.json({ message: "All fields are required" }, { status: 400 });
    }
    ...
    const result = await reviewsCollection.findOneAndUpdate(
      { _id: new ObjectId(id) },
      { $set: { customerName, reviewContent, rating, updatedAt: new Date() } },
